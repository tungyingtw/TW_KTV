...
function sanitizeSongSnapshot(snapshot, fallback = {}) {
  ...
  return {
    ...
    title,
    artist,
    lyricist: sanitizeText(snapshot.lyricist).slice(0, 120) || undefined,
    composer: sanitizeText(snapshot.composer).slice(0, 120) || undefined,
...
app.post('/api/report', async (req, res) => {
  ...
  const newReport = {
    ...
    lyricsSnippet: cleanLyricsSnippet || undefined,
...
async function normalizeAdminSongPayload(body, existingSong = null) {
  ...
  return {
    ...
    title,
    artist,
    lyricist: String(body.lyricist || '').trim() || undefined,
    composer: String(body.composer || '').trim() || undefined,
    language,
