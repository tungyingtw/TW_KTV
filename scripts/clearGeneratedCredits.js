...
function processFile(filePath, apply) {
  ...
  const stats = { filePath, songs: 0, removedLyricist: 0, removedComposer: 0, keptLyricist: 0, keptComposer: 0 };

  for (const [, song] of getSongEntries(payload)) {
    stats.songs++;
    if (isGeneratedCredit(song.lyricist, song.artist)) {
      delete song.lyricist;
      stats.removedLyricist++;
    } else if (song.lyricist) stats.keptLyricist++;

    if (isGeneratedCredit(song.composer, song.artist)) {
      delete song.composer;
      stats.removedComposer++;
    } else if (song.composer) stats.keptComposer++;
  }
...
try {
  ...
  if (args.help) {
    console.log('Usage: node scripts/clearGeneratedCredits.js [--apply] [--target server/database.json]');
    process.exit(0);
  }
  ...
  console.error(`[Clear Generated Credits] ${err.message}`);
  process.exit(1);
}
