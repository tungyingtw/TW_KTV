...
function processFile(filePath, apply) {
  ...
  const stats = { filePath, songs: 0, brandTotal: 0, removedTrue: 0, keptFalse: 0, missing: 0 };
  ...
  for (const [, song] of getSongEntries(payload)) {
    ...
    for (const status of Object.values(song.brands || {})) {
      ...
      if (status.available === true) {
        ...
        stats.removedTrue++;
      } else if (status.available === false) stats.keptFalse++;
      else stats.missing++;
    }
  }
...
try {
  ...
  if (args.help) {
    console.log('Usage: node scripts/clearRedundantAvailableTrue.js [--apply] [--target server/database.json]');
    process.exit(0);
  }
  ...
  console.error(`[Clear Redundant Available True] ${err.message}`);
  process.exit(1);
}
